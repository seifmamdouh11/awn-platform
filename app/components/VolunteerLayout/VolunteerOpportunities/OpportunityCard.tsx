...
export default function OpportunityCard({ opportunity, t, isArabic }: OpportunityCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col justify-between rounded-[2rem] border border-foreground/5 bg-background p-6 shadow-sm transition-all hover:shadow-xl hover:shadow-primary/5"
    >
          ...
          <div className="flex gap-2">
            ...
            {opportunity.event_type === "paid" && opportunity.reward !== undefined && (
              <span className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-emerald-600 border border-emerald-100 shadow-sm">
                {isArabic ? "مكافأة: " : "Reward: "} {opportunity.reward.toLocaleString()} EGP
              </span>
            )}
          </div>
          {/* Subtle ID */}
      ...
      <div className="mt-6 flex items-center justify-between">
        <Link
          ...
          className={`group/btn relative flex items-center gap-3 overflow-hidden rounded-full px-6 py-3 text-sm font-bold transition-all ${
            isClosed
