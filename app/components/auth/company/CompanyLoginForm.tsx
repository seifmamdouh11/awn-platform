...
import { Eye, EyeOff, Mail, Lock, Building2, ArrowRight, ArrowLeft, ShieldAlert } from "lucide-react";
...
export default function CompanyLoginForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      setLoading(true);
      const res = await api.post("/companies/login", data);
      ...
      if (code === "UNDER_REVIEW") {
        await Swal.fire({ icon: "info", title: t.underReviewTitle, text: t.underReviewText, confirmButtonText: t.ok });
        return;
      }
      if (code === "BLOCKED") {
        await Swal.fire({ icon: "error", title: t.blockedTitle, text: t.blockedText, confirmButtonText: t.ok });
        return;
      }
      if (status === 401) {
        await Swal.fire({ icon: "error", title: t.invalidCredentials, confirmButtonText: t.ok });
        return;
      }
      await Swal.fire({ icon: "error", title: t.loginFailed, confirmButtonText: t.ok });
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-2">
              <label htmlFor="co-email" className="text-sm font-semibold text-foreground/80">
            ...
            {/* Password */}
            <div className="space-y-2">
              <label htmlFor="co-password" className="text-sm font-semibold text-foreground/80">
            ...
            <div className="space-y-3 text-center pt-2">
              <p className="text-sm text-foreground/50">
                <Link href="/register/company" className="font-semibold text-foreground/70 hover:text-foreground underline underline-offset-4 transition">
                  {t.haveAccount}
                </Link>
              </p>
              <p className="text-sm text-foreground/40">
                {t.switchAccount}{" "}
                <Link href="/login/volunteer" className="font-semibold text-[#febc5a] hover:text-[#d97706] transition">
                  {t.switchAccountLink}
                </Link>
              </p>
            </div>
...
const translations = {
  en: {
    heroEyebrow: "Company Portal",
    heroTitle: "Empower your team,\nbuild real impact.",
    heroSubtitle: "Post opportunities, find passionate volunteers, and manage your social responsibility programs — all in one place.",
    heroQuote: "\"Great companies are built by people who give back.\"",
    stats: [
    ...
    subheadline: "Sign in to your company dashboard and manage your volunteer programs",
    formElements: { email: "Company email", password: "Password" },
    placeholders: { email: "company@example.com", password: "Enter your password" },
    errors: { required: "This field is required", invalidEmail: "Enter a valid email address" },
    haveAccount: "Don't have a company account? Register",
    switchAccount: "Are you a volunteer?",
    switchAccountLink: "Volunteer login →",
    login: "Sign in",
    loading: "Signing in...",
    ...
    underReviewText: "Your company account is pending approval. Please try again later.",
    blockedTitle: "Account blocked",
    blockedText: "Your company account has been blocked. Contact support.",
  ...
  ar: {
    ...
    stats: [
      { value: "+200", label: "شركة" },
      { value: "+10K", label: "متطوع" },
      { value: "+500", label: "فرصة" },
      { value: "98%", label: "رضا" },
    ],
    headline: "أهلاً بعودتك 👋",
    subheadline: "سجّل دخولك لإدارة برامج التطوع في شركتك",
    formElements: { email: "البريد الإلكتروني للشركة", password: "كلمة المرور" },
    placeholders: { email: "company@example.com", password: "اكتب كلمة المرور" },
    errors: { required: "هذا الحقل مطلوب", invalidEmail: "اكتب بريد إلكتروني صحيح" },
    haveAccount: "ليس لديك حساب شركة؟ أنشئ حساباً",
    switchAccount: "هل أنت متطوع؟",
    switchAccountLink: "← دخول المتطوعين",
    login: "تسجيل الدخول",
